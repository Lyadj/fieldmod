...
ColumnLayout{
    ...
    function handle_gripper_connect(){

        clickanimation.start()
        clickanimation_label.start()

        if(gripper_connectpanel.visible){
            gripper_connectpanel.close()
        }else{
            gripper_connectpanel.open()
        }
    ...
    function handle_gripper_state_changed(status){
        if(status){
            connectbtn.state="opened"
            status_indicator_string = gripper_backend.getSerialInfo()
        }else{
